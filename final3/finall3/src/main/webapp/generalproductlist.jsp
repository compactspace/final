...
$("li[class=group]").on("click",(e)=>{
	...
	// 클래스명과 id명에 당황마라! 부분 새로고침을 하려고 일부로 class="content2" id="content2" 이렇게 한다.
	// 스스로한테 로드 걸고 스스로한테 삽입하면 자기 자신으로 똑같이 삽입된다. 따라서 한번더 감싼후 자식
	//#content2 부분을 새로고침 하면된다.
	$(".content2").load(window.location.href+"?product_group="+product_group+" #content2");
...
<div class="content2">
		...
		<script>
	$(document).on("click","button[class=addbtn]", function(e) {		
		console.log($(this))
	...
	console.log("상품가격은->>"+$(this).parent().prev().prev().val()) 
		...
		//이건 레디스랑 상관없은 일반 상품 담기
		$.ajax({
			url : "generaladdcart.do",
			method : "get",
			data : {
